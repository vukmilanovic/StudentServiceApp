...
public class Add_Professor_To_Chair_Head extends JDialog{

	private CourseChair chair;
...
public Add_Professor_To_Chair_Head(CourseChair chair) {
		
		super();
		
		this.chair = chair;
		
		setTitle(getWord("addPToDepartment"));
		setSize(400,300);
		...
		while(it.hasNext())
			if(!(((it.next().getT() == Title.redovni_profesor) || (it.next().getT() == Title.vanredni_profesor)) && (it.next().getServiceyrs() >= 5)))
				it.remove();
        ...
        confirmButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				for(CourseChair cs : CourseChairDB.getInstance().getChairs()) {
					if(cs.getCode() == Add_Professor_To_Chair_Head.this.chair.getCode())
						break;
					idx++;
				}
				
				Edit_Department.OdustaniFlag = false;
				ProfessorController.getInstance().AddProfessorToCourseChairHead(idx, SelectedProf.getIDnum());
				dispose();
			}
		});
